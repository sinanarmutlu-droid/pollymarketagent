...
_proxy = __import__("os").environ.get("PROXY_URL","").strip()
if _proxy:
    _transport = httpx.HTTPTransport(proxy=httpx.Proxy(url=_proxy))
    _http_client = httpx.Client(http2=True, transport=_transport)
else:
